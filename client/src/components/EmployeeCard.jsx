    ...
    <div className="group relative card card-hover overflow-hidden">
        ...
        {!employee.isDeleted && (
            ...
            group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3">
                <button className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl
                shadow-lg transition-all hover:scale-105" onClick={()=>onEdit(employee)}>
                    <PencilIcon className="w-4 h-4" />
                </button>
                <button className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl
                shadow-lg transition-all hover:scale-105 disabled:opacity-50" onClick={handleDelete}>
