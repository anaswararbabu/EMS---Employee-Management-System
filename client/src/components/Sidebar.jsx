...
const Sidebar = () => {
        ...
        <>
            ...
            <div className='px-5 pt-6 pb-5 border-b border-white/6'>
                <div className='flex item-center justify-between'>
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                              <p className='font-semibold text-[13px] text-white tracking-wide'>Employee</p>
                              <p className='text-[11px] text-slate-500 font-medium'>Management System</p>
                        </div>
                    </div>
                    {/*CloseButton on Mobile */}
            ...
            {userName && (
                <div className='mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4'>
                    <div className='flex items-center gap-3'> 
                        <div className='w-9 h-9 rounded-lg bg-slate-800 flex items-center justify-center ring-1
                        ring-white/10 shrink-0'>
                ...
                <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                    {navItems.map((item)=>{
                        ...
                        return (
                            ...
                            rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? 
                            "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
            ...
            <div className='p-3 border-t border-white/6'>
                <button className='flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium
                 text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150'
  ...
  return (
    <>
        ...
        <button onClick={() => setMobileOpen(true)} className='lg:hidden fixed top-4 left-4 z-50 p-2 bg-slate-900 
        text-white rounded-lg shadow-lg border border-white/10'>
            <MenuIcon size={20} />
        </button>

        {/*Mobile Overlay */}
        {mobileOpen && <div className='lg:hidden fixed inset-0 bg-black/60 baackdrop-blur-sm z-40'
        onClick={()=> setMobileOpen(false)}/>}

        {/*Sidebar - desktop */}
        <aside className='hidden lg:flex flex-col h-full w-[260px] bg-linear-to-b from-slate-900 via-slate-900
        to-slate-950 text-white shrink-0 border-r border-white-4'>
            {sidebarContent}
        </aside>

        {/*Sidebar - mobile */}
        <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 to-slate-950 text-white
        z-50 flex flex-col transform transition-tranform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
            {sidebarContent}
        </aside>
    </>
  )
